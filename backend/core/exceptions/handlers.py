...
def custom_exception_handler(exc, context):
    ...
    if response is None:
        ...
        if request is not None:
            try:
                show_debug = (
                    request.GET.get('debug') == '1'
                    or (hasattr(request, 'data') and request.data.get('debug'))
                )
            except Exception:
                show_debug = False
        ...
        if show_debug:
            error_data["errors"]["traceback"] = traceback.format_exc()
            error_data["errors"]["type"] = type(exc).__name__
            error_data["errors"]["repr"] = repr(exc)
