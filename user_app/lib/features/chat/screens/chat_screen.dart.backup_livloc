...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget _messageContent(
    ...
    if (msgType == 'audio' && mediaUrls.isNotEmpty) {
      return _VoiceNotePlayer(url: mediaUrls.first.toString(), isMine: isMine);
    }
  ...
  Future<void> _loadApprovalStatus() async {
    try {
      ...
      if (status != _approvalStatus) {
        setState(() {
          _approvalStatus = status.isEmpty ? 'none' : status;
          _approvalId = data['approval_id']?.toString();
        });
      }
  ...
  Future<void> _requestBookingApproval() async {
    try {
      setState(() => _approvalStatus = 'pending');
      final res = await ChatAPI.requestApproval(widget.roomId);
      final data = (res['data'] ?? res) as Map;
      if (!mounted) return;
      setState(() {
        _approvalId = data['approval_id']?.toString();
        _approvalStatus = (data['status'] ?? 'pending').toString().toLowerCase();
      });
      _startApprovalPolling();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Request sent. Waiting for mechanic...')),
      );
    } catch (e) {
      if (!mounted) return;
      setState(() => _approvalStatus = 'none');
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      case 'approved':
        return Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: Text(
              'You can now book this mechanic!',
      ...
      case 'rejected':
        return Container(
          ...
          child: Row(children: [
            ...
            TextButton(
              onPressed: _requestBookingApproval,
  ...
  Future<void> _shareLocation() async {
    ...
    try {
      ...
      // 3. Tuma kwa backend
      final res = await ChatAPI.shareLocation(
        roomId: widget.roomId,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(km != null
              ? 'Location imetumwa ✅ (Umbali: ${km}km)'
              : 'Location imetumwa ✅'),
          backgroundColor: Colors.green,
        ),
      );
