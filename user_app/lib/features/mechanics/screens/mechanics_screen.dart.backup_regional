...
class _MechanicsScreenState extends State<MechanicsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      debugPrint('[Mechanics] user=$user');
      debugPrint('[Mechanics] userRegion="$userRegion"');

      // Kama user hana region — REFRESH profile kutoka backend kwanza
      String? regionFilter = (userRegion != null && userRegion.trim().isNotEmpty)
          ? userRegion.trim()
          : null;
      ...
      if (regionFilter == null) {
        try {
          ...
          final freshRegion = profile['region']?.toString();
          debugPrint('[Mechanics] fresh profile region="$freshRegion"');
          if (freshRegion != null && freshRegion.trim().isNotEmpty) {
            regionFilter = freshRegion.trim();
            // Update stored user
            await AuthState.instance.updateUser(profile);
          }
        } catch (e) {
          debugPrint('[Mechanics] profile refresh failed: $e');
        }
      }

      debugPrint('[Mechanics] Filtering by region: "$regionFilter"');
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // INFO MESSAGE — eleza kwa nini ni chat kwanza
          Container(
            padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    'We want you firstly to start a short conversation with our mechanic for more explanation. '
                    'After reaching an agreement, the mechanic will allow you to book.',
                    style: GoogleFonts.poppins(
                      fontSize: 12,
                      height: 1.5,
